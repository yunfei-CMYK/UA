...
class Config:
    def __init__(self):
        ...
        self.batch_size = 32
        self.epochs = 100
        self.model_save_path = './saved_model/bilstm_crf.pt'
        self.log_file = '../result/bilstm_crf.txt'
...
# 训练模型并保存日志
def train(model, train_loader, optimizer, epochs, model_save_path, log_file):
    ...
    with open(log_file, 'w') as f_log:
        ...
        for epoch in range(epochs):
            ...
            # 在每个 epoch 结束后进行评估
            all_tags, all_preds = evaluate(model, train_loader)
            ...
            # 记录日志
            f_log.write(f'{epoch + 1}, {total_loss:.6f}, {accuracy:.6f}, {precision:.6f}, {recall:.6f}, {f1:.6f}\n')
...
def main():
    ...
    if os.path.exists(config.model_save_path):
        ...
        train(model, train_loader, optimizer, epochs=config.epochs, model_save_path=config.model_save_path,
